...
class Grammar:
    ...
    def add_rule(self, left: str, right: List[str]):
        if left not in self.non_terminals:
            raise GrammarException(f"Left side non-terminal '{left}' is not declared in NonTerminals.")

        for sym in right:
            if sym not in self.non_terminals and sym not in self.terminals:
                raise GrammarException(f"Unknown symbol '{sym}' in rule {left} -> {''.join(right)}")
        
        self.rules[left].append(right)

    def __str__(self):
        res = f"Start: {self.start_symbol}\n"
        for left, rights in self.rules.items():
            for right in rights:
                r_str = "".join(right) if right else "ε"
                res += f"{left} -> {r_str}\n"
        return res
...
class EarleyItem:
    __slots__ = ('head', 'body', 'dot', 'start', '_hash')
    ...
    def __repr__(self):
        ...
        return f"[{self.head} -> {''.join(body_list)}, {self.start}]"
...
class EarleyParser(ParsingAlgorithm):
    ...
    def predict(self, word: str) -> bool:
        ...
        predicted_in_column = [set() for _ in range(n + 1)]
            ...
            while idx < len(Q[j]):
                ...
                if not item.is_complete():
                    ...
                    if next_sym in self.grammar.non_terminals:
                        self._predict(next_sym, j, D, Q, predicted_in_column)
    ...
    def _predict(self, non_terminal, current_pos, D, Q, predicted_in_column):
        if non_terminal in predicted_in_column[current_pos]:
            return
        
        predicted_in_column[current_pos].add(non_terminal)
